...
const DEFAULT_PIN_VH_MULTIPLE = 2.4;
...
export type HeroScrubProps = {
  ...
  holdImmersion?: boolean;
  pinHeightMultiple?: number;
  id?: string;
};
...
export function HeroScrub({
  ...
  defaultAspect = 16 / 9,
  holdImmersion = true,
  pinHeightMultiple = DEFAULT_PIN_VH_MULTIPLE,
  id,
}: HeroScrubProps) {
  ...
  // Load single image or multi-frame sequence
  useEffect(() => {
    if (reduced) return;
  ...
  }, [reduced, frameCount, frameUrl, imgUrl]);
  ...
  // Scroll-driven choreography with GSAP ScrollTrigger
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const drawFrame = (index: number) => {
        ...
        if (!isLoaded(useIdx)) {
          let found = -1;
          for (let d = 1; d < (frameCount || 0); d++) {
            if (useIdx - d >= 0 && isLoaded(useIdx - d)) { found = useIdx - d; break; }
            if (useIdx + d < (frameCount || 0) && isLoaded(useIdx + d)) { found = useIdx + d; break; }
          }
          if (found === -1) return;
          useIdx = found;
        }
      ...
      // 1. Zoom from initial scale and split titles apart horizontally
      ...
      // 2. Expand card to immerse and cover the entire screen
      master.to(cardRef.current, { scale: immerseScale(), ease: "power2.in", duration: 0.55 }, 0.2);
      master.to(titleTopRef.current, { opacity: 0, ease: "power1.in", duration: 0.2 }, 0.2);
      master.to(titleBottomRef.current, { opacity: 0, ease: "power1.in", duration: 0.2 }, 0.2);

      // 3. If not holding immersion (e.g. standalone demo), scale back down at the end
      if (!holdImmersion) {
    ...
    return () => ctx.revert();
  }, [ready, framesOk, reduced, aspect, frameCount, frameUrl, imgUrl, holdImmersion]);

  const tallHeight = `${(pinHeightMultiple + 1) * 100}vh`;
  ...
  return (
    ...
    >
      ...
      >
        <div ref={bgRef} aria-hidden className="absolute inset-0 z-0 opacity-40 transition-colors duration-700" style={{ backgroundColor: accentHex }} />
        ...
        <div className="relative z-10 flex h-full w-full flex-col items-center justify-center gap-3 md:gap-4">
          <h2
            ref={titleTopRef}
            aria-hidden
            className="font-black uppercase font-cinzel select-none text-center px-4"
            style={{
          ...
          <div
            ref={cardRef}
            className="relative overflow-hidden rounded-[16px] shadow-[0_25px_90px_rgba(0,0,0,0.85)] ring-1 ring-white/15 will-change-transform md:rounded-[24px]"
            style={{
          ...
          >
            <div aria-hidden className="pointer-events-none absolute inset-0 z-20 shadow-[inset_0_0_80px_rgba(0,0,0,0.5)]" />
            {imgUrl ? (
              <img
                src={imgUrl}
